<?php
class PgSchema extends CPgsqlSchema
{
    public    $_currentSchema;
    ...
    function __construct($conn)
    {
        ...
        $this->_currentSchema=$conn->schema;
    }
    ...
    protected function resolveTableNames($table,$name)
    {
        ...
        {
            $schemaName=$this->_currentSchema;
            $tableName=$parts[0];
        }

        $table->name=$tableName;
        $table->schemaName=$schemaName;
        if($schemaName===$this->_currentSchema)
...
EOD;
        ...
        foreach($columns as $column)
        {
            ...
            if(stripos($column['adsrc'],'nextval')===0 && preg_match('/nextval\([^\']*\'([^\']+)\'[^\)]*\)/i',$column['adsrc'],$matches))
            {
                ...
                if(strpos($matches[1],'.')!==false || $table->schemaName===$this->_currentSchema)
