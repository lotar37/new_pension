...
function checkDate(str){
	...
	var str_first = str2[0] +'-'+ str2[1]+'-'+ str2[2];
	
//alert(str2 + " - " + year_delta_top+ " - " + year_delta_bottom + " - " + (new Date(0+str2[2],str2[1]-1,0+str2[0])));
	var date1 = new Date(0+str2[2],str2[1]-1,0+str2[0]);
	var str_second = check(date1.getDate()) + "-" + check(date1.getMonth()+1) + "-" + date1.getFullYear();
//	alert(str_first + " --- " + str_second)
	if((str_first !== str_second) || year_delta_top<0 || year_delta_bottom<0)return -1;
	return 1;
}
function dateDifference(date1,date2,nvl){
	var a_month = [31,31,28,31,30,31,30,31,31,30,31,30,31];
...
$("#sen_cal_name").blur(function(){
	if($(this).val == "")$(this).addClass("wrong_date");
	else $(this).removeClass("wrong_date");
});
function getNVL(){
	var nvl = 1;
    ...
    $.ajax({
        ...
        success: function (data) {
			if(data.status == "ok"){
				...
				$("tr").removeClass("error_date");
				$("#sen_cal_name").addClass("wrong_date");
			...
			else if(data.status == "out_date"){
				alert("Период выходит за время жизни пенсионера.")
			}
        }
    });        
	
});
$("#sen_cal_add, #sen_cal_del").mouseover(function(){
	$(this).css("color","yellow");
});
$("#sen_cal_add, #sen_cal_del").mouseout(function(){
...
if(isset($model->rank))echo Ranks::getName($model->rank)." "; 
...
<td id='sen_cal_m'></td>
<td id='sen_cal_d'></td>
<td id='sen_cal_add' style='padding-right:3px;display:none;cursor:pointer'>Сохр</td>
